...
{
    return [self.downloadDirectory stringByAppendingPathComponent:@"downloadsFileSize.plist"];
}
...
- (void)startWithDownloadModel:(TYDownloadModel *)downloadModel
{
    ...
    if (downloadModel.state == TYDownloadStateWaiting) {
        [self downloadModel:downloadModel didChangeState:TYDownloadStateWaiting filePath:nil error:nil];
        return;
    }
    ...
    if (downloadModel.task && downloadModel.task.state == NSURLSessionTaskStateRunning) {
        downloadModel.state = TYDownloadStateDownloading;
        [self downloadModel:downloadModel didChangeState:TYDownloadStateDownloading filePath:nil error:nil];
        return;
    }
...
- (BOOL)canResumeDownlaodModel:(TYDownloadModel *)downloadModel
{
    ...
    @synchronized (self) {
        if (self.downloadingModels.count >= _maxDownloadCount ) {
            ...
            downloadModel.state = TYDownloadStateWaiting;
            [self downloadModel:downloadModel didChangeState:TYDownloadStateWaiting filePath:nil error:nil];
            return NO;
        }
...
{
    if (!downloadModel.task && downloadModel.state == TYDownloadStateWaiting) {
